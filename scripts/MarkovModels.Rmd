...
```{r}
pmat = pmatrix.msm(msm.l1_priority, ci = "normal")
Testimates = data.frame(pmat$estimates)
TTable = gather(Testimates, key = "to")
TTable$from = rownames(Testimates)
TTable$U = c(pmat$U)
TTable$L = c(pmat$L)

write.csv(x = TTable, "../data/trans_estimates.csv")
```
...
native_trans = p %>%
  ...
  #geom_ribbon(aes(ymax = Upper, ymin = Lower), alpha = .2) +
  facet_grid(~priority) +
  ggtitle("A) Priority Effects on Transition Probability") +
  scale_color_manual(name = 'Previous State Assignment', 
                     values =c("State 2" = cols[2], 
                               "State 3" = cols[3], "State 4" = cols[4]),
                     labels = c('2. F. perennis - B. hordeaceous', 
  ...
  scale_linetype_manual(name = 'Previous State Assignment',
                        ...
                        labels = c('2. F. perennis - B. hordeaceous', 
                                   ...
                                   '4. A. fatua - B. diandrus')) +
  xlab("SPEI") +
  ylab("State 1 Transition Probability") +
  ylim(0, .5)
