...
data and classifying community types?

2. Define questions clearly - what exactly is this approach answering?
...
qmatrix.msm(msm.env_priority, covariates=list(net_cum_3 = 100))
p12 = 1 - exp(-.02581)
p13 = 1 - exp(-.02605)
p14 = 1 - exp(-.1894)
tot = sum(p12,p13,p14)
...
for(pri in c(0:1)){
  for(netprecip in seq(-100, 500, by = 100)){
    qm = as.matrix(qmatrix.msm(msm.env_priority, covariates=list(net_cum_3 = netprecip, 
                                                               natives = pri, annuals = pri, waps = pri))$estimates)
    ql = as.matrix(qmatrix.msm(msm.env_priority, covariates=list(net_cum_3 = netprecip, 
                                                                 natives = pri, annuals = pri, waps = pri))$L)
    qu = as.matrix(qmatrix.msm(msm.env_priority, covariates=list(net_cum_3 = netprecip, 
...
bind_rows(Nprob) %>%
  ...
  facet_wrap(~priority) +
  ggtitle("Probability of Stability") +
  scale_color_manual(name = 'Initial State', 
         values =c("State 1" = cols[1], "State 2" = cols[2], 
                   "State 3" = cols[3], "State 4" = cols[4]),
         labels = c('Natives','Priority Annuals', 'WAPS','Dry Annuals')) +
  scale_fill_manual(name = 'Initial State', 
         values =c("State 1" = cols[1], "State 2" = cols[2], 
                   "State 3" = cols[3], "State 4" = cols[4]),
         labels = c('Natives','Priority Annuals', 'WAPS','Dry Annuals')) +
  xlab("Net Precipitation Deficit") +
  ylab("Probability of Retaining State in Next Year")

# ggsave("../figures/StabilityProbabilities.pdf", height = 6, width = 8)
...
bind_rows(Nprob) %>%
  filter(From != To) %>%
  mutate(priority = if_else(priority == 1, "With Priority", "No Priority")) %>%
  ggplot(aes(x = precip,
             color = From,
             fill = From)) +
  geom_line(aes(y = Trans), size = 1.5) +
  geom_ribbon(aes(ymax = Upper, ymin = Lower), alpha = .2) +
  facet_grid(To~priority) +
  ...
  scale_fill_manual(name = 'Initial State', 
         ...
         labels = c('Natives','Priority Annuals', 'WAPS','Dry Annuals')) +
  xlab("Net Precipitation Deficit") +
  ylab("Probability of Transition in Next Year")

ggsave("../figures/TransitionProbabilities.pdf", height = 14, width = 8)
```
